...
class SW2DroneController extends ControlExtension {
    ...
    public void onObjectClick(ControlObjectClickEvent event) {
        ...
        Log.d(DroneService.LOG_TAG, "is long: "
                + (event.getClickType() == Control.Intents.CLICK_TYPE_LONG));
...
//                e.printStackTrace();
    ...
    private void toggleMode() {
        ...
        try {
            if (flyMode) {
                /** Freeflight
                 * According to the documentation in ControlDroneActivity, the
                 * needed combination is
                 * - progressiveCommandEnabled to TRUE
                 * - progressiveCommandCombinedYawEnabled to TRUE
                 * - running to TRUE
                 */
                Log.d(DroneService.LOG_TAG, "Flymode TRUE, setProgressIveCommandEnabled and setProgressiveCommandCombinedYawEnabled to TRUE");
                ControlDroneActivity.droneControlService.setProgressiveCommandEnabled(true);
                ControlDroneActivity.droneControlService.setProgressiveCommandCombinedYawEnabled(true);
                ControlDroneActivity.combinedYawEnabled = true;
                ControlDroneActivity.running = true;
            } else {
                // Stop all movement
                ...
                ControlDroneActivity.droneControlService.setProgressiveCommandEnabled(false);
                ControlDroneActivity.droneControlService.setProgressiveCommandCombinedYawEnabled(false);
                ControlDroneActivity.combinedYawEnabled = false;
                ControlDroneActivity.running = false;
            }

            // Currently, setting CommandEnabled and CombinedYawEnabled to true
            // enables SW1 and SW2 control. Setting CommandEnabled to flyMode
            // and CombinedYawEnabled to false enables SW2 control only.
            ControlDroneActivity.droneControlService
                    .setProgressiveCommandEnabled(true);
            ControlDroneActivity.droneControlService
        ...
        int imageRes = flyMode ? R.drawable.state_move
                : R.drawable.state_control;
        ...
        public void run() {
            while (runTimerThread) {
                ...
                float currentTime = (System.currentTimeMillis() - startTime)
                        / (float) 1000;
                if (isFlying)
                    sendText(R.id.time_text,
                            String.format("%.2fs", currentTime));
            }
        }
    };
}
